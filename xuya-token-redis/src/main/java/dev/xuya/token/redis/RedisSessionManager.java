 ...
 * 会话 key 形如 {@code {prefix}{token}},用户索引为集合 key
 * {@code {prefix}user:{userId}},支持并发会话限制、踢人下线与在线列表。
 *
 * @author 青衣
 */
public class RedisSessionManager implements SessionManager {
    ...
    /**
     * 为用户创建新会话并写入用户索引;启用并发限制时清理索引中的
     * 失效成员,超限按策略踢最旧会话或抛出 {@code ForbiddenException} 拒绝。
     */
    @Override
    public Session create(String userId) {
        ...
        SessionData data = new SessionData(token, userId, now.toEpochMilli(), timeoutMillis);
        ...
        if (maxSessionsPerUser > 0 && !enforceLimit(userId, userKey, token)) {
            // 被拒绝登录:回滚刚写入的会话与索引
    ...
    public void invalidate(String token) {
        ...
        redisTemplate.delete(key(token));
        if (data != null) {
            redisTemplate.opsForSet().remove(userKey(data.userId), token);
        }
    }

    /** 注销该用户的全部会话(踢人下线):逐个删除会话 key 后删除索引集合。 */
    @Override
    public void invalidateByUserId(String userId) {
        String userKey = userKey(userId);
        Set<String> members = readMembers(userId);
    ...
    /** 该用户当前未过期的 token 集合:以会话 key 是否存在为准过滤索引成员。 */
    @Override
    public Set<String> listActiveTokens(String userId) {
        Set<String> result = new HashSet<>();
        for (String token : readMembers(userId)) {
     ...
     */
    private boolean enforceLimit(String userId, String userKey, String newToken) {
        Set<String> members = readMembers(userId);
    ...
    /** 读取并反序列化会话数据;不存在或数据损坏返回 {@code null}。 */
    ...
    /** 读取用户索引的全部成员;索引不存在返回空集。 */
    private Set<String> readMembers(String userId) {
        Set<String> members = redisTemplate.opsForSet().members(userKey(userId));
        return members == null ? Set.of() : members;
    }

    /** 会话 key:{@code {prefix}{token}}。 */
    private String key(String token) {
        return keyPrefix + token;
    }

    /** 用户索引 key:{@code {prefix}user:{userId}}。 */
    ...
    static final class SessionData {
        ...
        SessionData(String token, String userId, long createdAtMillis, long timeoutMillis) {
            this.token = token;
            this.userId = userId;
